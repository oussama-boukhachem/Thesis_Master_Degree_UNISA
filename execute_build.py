...
if __name__ == '__main__':
    ...
    if thesis_pkg.exists():
        ...
        for item in sorted(pkg_contents)[:10]:
            print(f'       - {item.name}')
            
    if github_ws.exists():
        ws_contents = list(github_ws.iterdir())
        print(f'[INFO] github_thesis_workspace contents ({len(ws_contents)} items):')
        for item in sorted(ws_contents)[:10]:
            if item.name not in ['thesis-github-package', 'build_thesis_structure.py', 'run_build.bat', 'execute_build.py']:
                print(f'       - {item.name}')
...
''',
    ...
    print(f'Directory structure verified: {all_dirs_exist}')
    print('='*70)
